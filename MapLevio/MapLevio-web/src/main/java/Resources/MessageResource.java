...
public class MessageResource {
	...
	public Response getMessageByType(@QueryParam(value="typeMsg")String typeMessage) {
		...
		/******Messages non lu******/
		if( typeMessage.equals(MsgNonLu))
		
		return Response.status(Status.OK).entity(MessageMetier.getAllMessageNonLu()).build();
		
		/******Messages archive******/
		if( typeMessage.equals(Archive))

			return Response.status(Status.OK).entity(MessageMetier.getAllMessageArchiver()).build();
	...
	@Path("/findBy/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response FindMsgById(@PathParam(value="id") int id) {
		Message msg=MessageMetier.FindMessageById(id);
		MessageMetier.GetMessageBody(msg);
